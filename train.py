...
loss_types = ['B', 'BIoU', 'C', 'C_sem', 'stuff', 'M', 'MIoU', 'T', 'center', 'B_shift', 'M_shift', 'M_occluded',
              'P', 'D', 'S', 'I']
...
def train():
    ...
    if args.resume is not None:
        ...
        net.init_weights(backbone_path='weights/pretrained_models_coco/' + cfg.backbone.path, local_rank=args.local_rank)
    ...
    try:
        for epoch in range(cfg.max_epoch):
            ...
            for i, data_batch in enumerate(data_loader):
                ...
                preds = net(imgs)
                losses = criterion(preds, gt_bboxes, gt_labels, gt_masks, gt_ids)
...
def compute_validation_map(yolact_net, valid_data=False, device=0, output_metrics_file=None):
    with torch.no_grad():
        yolact_net.eval()
        print()
        print("Computing validation mAP (this may take a while)...", flush=True)
        metrics = eval_script.validation(yolact_net, device=device,
