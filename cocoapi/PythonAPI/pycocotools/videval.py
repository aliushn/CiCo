...
class VIDeval:
    ...
    def __init__(self, vidGt=None, vidDt=None, iouType='bbox', output_file=None):
        '''
        Initialize CocoEval using coco APIs for gt and dt
        :param vidGt: coco object with ground truth annotations
        :param vidDt: coco object with detection results
        :return: None
        '''
        if not iouType:
            print('iouType not specified. use default iouType segm')
        self.vidGt = vidGt  # ground truth VID API
        self.vidDt = vidDt  # detections VID API
        ...
        if vidGt is not None:
            self.params.vidIds = sorted(vidGt.getVidIds())
            self.params.catIds = sorted(vidGt.getCatIds())
    ...
    def _prepare(self):
        ...
        gts = self.vidGt.dataset['annotations']
        dts = self.vidDt.dataset['annotations']
    ...
    def evaluate(self):
        ...
        self.ious = {(vidId, catId): self.computeIoU(vidId, catId) \
                     for vidId in p.vidIds
                     for catId in catIds}

        maxDet = p.maxDets[-1]

        self.evalImgs = [self.evaluateVid(vidId, catId, areaRng, maxDet)
                         for catId in catIds
    ...
    def computeIoU(self, vidId, catId):
        ...
        def iou_seq(d_seq, g_seq):
            ...
            for d, g in zip(d_seq, g_seq):
                if d and g:
                    i_cur = max(min(d[2], g[2]) - max(d[0], g[0]), 0) * max(min(d[3], g[3]) - max(d[1], g[1]), 0)
                    i += i_cur
                    u += (d[2]-d[0])*(d[3]-d[1]) + (g[2]-g[0])*(g[3]-g[1]) - i_cur
                elif not d and g:
                    u += (g[2]-g[0])*(g[3]-g[1])
                elif d and not g:
                    u += (d[2]-d[0])*(d[3]-d[1])
...
class Params:
    ...
    def setDetParams(self):
        ...
        self.iouThrs = np.linspace(.5, 0.95, np.round(int((1.00 - .5) / .05)), endpoint=True)
        self.recThrs = np.linspace(.0, 1.00, np.round(int((1.00 - .0) / .01)) + 1, endpoint=True)
