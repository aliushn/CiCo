...
class CandidateShift(object):
    def __init__(self, net, correlation_patch_size=5, train_maskshift=False, train_masks=True, track_by_Gaussian=False,
                 update_track=False, proto_coeff_occlusion=False):
        """
    ...
    def __call__(self, candidate, ref_candidate, img=None, img_meta=None):
        fpn_feat_next = candidate['fpn_feat'].unsqueeze(0) if candidate['fpn_feat'].dim()==3 else candidate['fpn_feat']
        fpn_feat_ref = ref_candidate['fpn_feat'].unsqueeze(0) if ref_candidate['fpn_feat'].dim()==3 else ref_candidate['fpn_feat']
        ...
        if ref_candidate['box'].size(0) > 0:
            ...
            if display:
                display_box_shift(box_ref, ref_candidate['box'], img_meta=img_meta, img_gpu=img)
            ...
            if self.train_masks:
                ...
                if self.proto_coeff_occlusion:
                    ref_candidate['mask'] = pred_masks_all[:, 0] - pred_masks_all[:, 1]
                    ref_candidate['mask_non_target'] = pred_masks_all[:, 1]
                else:
                    ref_candidate['mask'] = pred_masks_all
